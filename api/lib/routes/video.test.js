...
function createAndExpectSampleVideo(sample, toObject) {
  const deferred = sendAndExpectReq('POST', '/video', { payload: sample })
    .expect(({ body: { data }}) => {
...
function expectSampleVideoById(id, expectedStatus) {
  return sendAndExpectReq('GET', `/video/${id}`, { expectedStatus });
}
...
describe('Route /video', () => {
  ...
  test('It should return all videos', async () => {
    ...
    await sendAndExpectReq('GET', '/video')
      .expect(({ body: { data }}) => {
  ...
  test('It should update a video', async () => {
    ...
    await sendAndExpectReq('PUT', `/video/${video1.id}`, { payload: videoUpdate })
      .expect(({ body: { data }}) => {
  ...
  test('It should delete a video', async () => {
    ...
    await sendAndExpectReq('DELETE', `/video/${video.id}`)
      .expect({ data: 1 });
